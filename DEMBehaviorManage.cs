...
namespace Cobra.KALL06
{
    internal class DEMBehaviorManage
    {
        ...
        public UInt32 Read(ref TASKMessage msg)
        {
            ...
            //bug 15978
            if (msg.gm.sflname == "Trim")  //如果是Scan/Trim SFL
            {
        ...
        */
        ...
        {
            string msg = "OZ7706 EFUSE Download:\n";
            msg += "\tAddress\tValue\n";
            UInt32 ret = LibErrorCode.IDS_ERR_SUCCESSFUL;
            ...
            for (byte badd = (byte)ElementDefine.EF_USR_OFFSET; badd <= (byte)ElementDefine.EF_USR_TOP; badd++)
            {
                ...
                if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL)
                {
                    msg += $"\t0x{badd.ToString("X2")}\t0x{EFUSEUSRbuf[badd - (byte)ElementDefine.EF_USR_OFFSET].ToString("X4")}\tFailed.\n";
                    FolderMap.WriteFile(msg);
                    return ret;
                }
                msg += $"\t0x{badd.ToString("X2")}\t0x{EFUSEUSRbuf[badd - (byte)ElementDefine.EF_USR_OFFSET].ToString("X4")}\n";
            }
            FolderMap.WriteFile(msg);
        ...
        {
            string msg = "OZ7706 EFUSE Read Back Check:\n";
            msg += "\tAddress\tWrite\tRead\n";
            UInt32 ret = LibErrorCode.IDS_ERR_SUCCESSFUL;
#if readback
            return LibErrorCode.IDS_ERR_DEM_BUF_CHECK_FAIL;
#else
            ...
            for (byte badd = (byte)ElementDefine.EF_USR_OFFSET; badd <= (byte)ElementDefine.EF_USR_TOP; badd++)
            {
                ...
                if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL)
                    return ret;
                msg += $"\t0x{badd.ToString("X2")}\t0x{EFUSEUSRbuf[badd - ElementDefine.EF_USR_OFFSET].ToString("X4")}\t0x{pval.ToString("X4")}\n";
                if (pval != EFUSEUSRbuf[badd - ElementDefine.EF_USR_OFFSET])
                {
                    msg += "Failed.";
                    FolderMap.WriteFile(msg);
        ...
        public UInt32 GetDeviceInfor(ref DeviceInfor deviceinfor)
        {
            ...
            ret = ReadWord(0x75, ref wval);
            if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL) return ret;
            if ((wval & 0x4000) == 0x0000)
        ...
        public UInt32 GetSystemInfor(ref TASKMessage msg)   //dic中的index，对应参数实际地址。dic中的key，对应显示的位置。dic中的value，对应是否显示。
        {
            UInt32 ret = LibErrorCode.IDS_ERR_SUCCESSFUL;
            msg.sm.dic.Clear();
            UInt32 cellnum = (UInt32)parent.CellNum.phydata + 4;    //“00” ~ “10” corresponds to 4 ~ 6 cells.
